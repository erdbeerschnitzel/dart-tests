...
WSClient.prototype.run = function() {
  var ws = _WebSocketFactoryProvider.WebSocket$factory("ws://echo.websocket.org");
  ws.get$on().get$message().add($wrap_call$1(this.get$onMessage()), false);
  ws.send("this is dart!");
  this.write("engage!");
}
WSClient.prototype.onMessage = function(event) {
  this.write(event.get$data());
}
WSClient.prototype.get$onMessage = function() {
  return this.onMessage.bind(this);
}
WSClient.prototype.write = function(message) {
